...
// import { Clock, AlertCircle, Pause, Camera, Monitor, Shield } from "lucide-react";
...
//                         value={option}
//                         checked={currentQuestion.multipleAnswers 
//                           ? answers[currentQuestion.id]?.includes(option)
//                           : answers[currentQuestion.id] === option
//                         }
//                         onChange={(e) => {
//                           if (currentQuestion.multipleAnswers) {
//                             // Handle multiple answers for checkbox
...
//                           } else {
//                             // Handle single answer for radio buttons
...
//           {/* Bottom Navigation - REMOVED PREVIOUS BUTTON */}
...
interface ExamData {
  ...
  roomId: string;
  roomName: string;
  googleFormLink?: string; // Added this field
}
...
const TakeExam = () => {
  ...
  const saveExamResults = () => {
    ...
    const results = {
      ...
      totalQuestions: examData.questions.length,
      answeredQuestions: Object.keys(answers).length,
      googleFormLink: examData.googleFormLink
    };
  ...
  const hasGoogleForm = examData.googleFormLink && examData.googleFormLink.trim() !== "";
    ...
    <div className="h-screen flex flex-col bg-white overflow-hidden select-none">
      ...
      <div className={`${mainTimer <= 60 && !isOnBreak ? 'bg-red-600' : 'bg-slate-800'} text-white px-6 py-3 flex items-center justify-between border-b-2 ${mainTimer <= 60 && !isOnBreak ? 'border-red-700' : 'border-slate-900'} flex-shrink-0 transition-colors duration-300`}>
        <div className="flex items-center gap-4">
          ...
          {!hasGoogleForm && currentQuestion && currentQuestion.timeLimit > 0 && (
            <>
              <div className="w-px h-5 bg-slate-600"></div>
              <div className="flex items-center gap-2">
                <AlertCircle className="w-4 h-4 text-amber-400" />
        ...
        <div className="text-sm font-semibold">
          {hasGoogleForm ? "Google Form Exam" : `Question ${currentQuestionIndex + 1} of ${examData.questions.length}`}
        </div>
      </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {hasGoogleForm ? (
            ...
            <div className="flex-1 flex flex-col p-8 overflow-hidden">
              <div className="flex items-center justify-between mb-4 pb-4 border-b-2 border-slate-200 flex-shrink-0">
                <div>
                  <h2 className="text-2xl font-semibold text-slate-900">{examData.examName}</h2>
                  <p className="text-sm text-slate-600 mt-1">{examData.examSubtitle}</p>
                </div>
              </div>
               console.log(examData.googleFormLink);
              <div className="flex-1 border-2 border-slate-300 rounded overflow-hidden">
               
                
                <iframe
                  src={examData.googleFormLink}
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              <div className="flex-1 flex flex-col p-8 overflow-auto">
                ...
                <div className="flex items-start justify-between mb-6 pb-4 border-b-2 border-slate-200 flex-shrink-0">
                  ...
                  <div className="text-sm font-bold text-slate-900 border-2 border-slate-800 px-4 py-2 flex-shrink-0">
                    [{currentQuestion?.marks} Mark{currentQuestion && currentQuestion.marks > 1 ? "s" : ""}]
                  </div>
                </div>
          ...
          {/* Submit Button for Google Form */}
          {hasGoogleForm && (
            <div className="border-t-2 border-slate-200 bg-slate-50 px-8 py-4 flex items-center justify-between flex-shrink-0">
              <div className="text-sm text-slate-600">
                <span className="font-semibold">Complete the form and submit when ready</span>
              </div>
              <div className="flex gap-3">
                <Button
                  onClick={handleBreak}
