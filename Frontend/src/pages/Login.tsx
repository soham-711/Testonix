...
export default function Login() {
  ...
  const joinCode = localStorage.getItem("invitetoken") // <-- from JoinRoomPage
console.log(joinCode);

  /* -------------------------------------------------
   * 1. Wait for Firebase auth init & auto-redirect
   * ------------------------------------------------- */
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, (firebaseUser) => {
      setAuthReady(true);
      if (firebaseUser) {
        // Already logged in → go where we wanted
        if (joinCode) {
          navigate(`/join-room/${joinCode}`, { replace: true });
        } else {
          navigate("/dashboard", { replace: true });
        }
      }
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      await axios.post("http://localhost:5000/api/users/createuser", userData);
      localStorage.setItem("uid", firebaseUser.uid);

      // Redirect based on the original invite
      if (joinCode) {
        navigate(`/join-room/${joinCode}`, { replace: true });
      } else {
        navigate("/dashboard", { replace: true });
  ...
  /* -------------------------------------------------
   * 3. Show a tiny loader until Firebase is ready
   * ------------------------------------------------- */
  if (!authReady) {
    return (
  ...
  /* -------------------------------------------------
   * 4. UI (unchanged, just a few tiny clean-ups)
   * ------------------------------------------------- */
  return (
    <div
      className="min-h-screen flex items-center justify-center relative overflow-hidden select-none"
    ...
    >
      ...
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-blue-300/20 via-transparent to-transparent" />
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_left,_var(--tw-gradient-stops))] from-cyan-300/20 via-transparent to-transparent" />
        ...
        <div className="text-center space-y-6">
          ...
          <div className="flex justify-center select-none">
            <div className="relative group">
              <div className="absolute inset-0 bg-gradient-to-r from-blue-400 to-cyan-400 rounded-full blur-xl opacity-50 group-hover:opacity-70 transition-opacity duration-300" />
              <img
                src={logoImage}
                alt="Examly Logo"
                className="w-32 h-32 object-contain relative z-10 drop-shadow-2xl transform group-hover:scale-105 transition-transform duration-300 select-none"
